          ...
          begin
              ...
              begin
                  if Length(edtSetor.Text) < edtSetor.MaxLength then
                   begin
                       Informacao('Grupo deve conter '+IntToStr(edtSetor.MaxLength)+' dígitos!','Aviso!');
                       edtSetor.SetFocus;
                       Abort;
                       Exit;
                   end;
                  ...
                  BEGIN   }
                      ...
                      begin
                          Informacao('Para acessar o Financeiro o'+#13+
                                    ' código do usuário deve ser numerico!!'+#13,'Aviso!');
                          edtSetor.SetFocus;
                          Abort;
                          Exit;
                ...
                {      Informacao('Setor não está cadastrado para'+#13+
                                 'o código '+dtmSistema.sCod_UsuarioNL+' !'+#13,'Aviso!');
                      edtSetor.SetFocus;
                      Abort;
                      Exit;
                  ...
                  if dtmSistema.QryConsulta.RecordCount < 1 then
                  begin
                      Informacao('Setor não está cadastrado ou'+#13+
                                 'não possui itens em livro!!!'+#13,'Aviso!');
                      edtSetor.SetFocus;
                      Abort;
                      Exit;
                  end else
       ...
       // marca a query para a linha selecionada
        ...
        begin
         if not Pergunta('Deseja realmente aprovar esta quantidade?'+#13+
                   'Loja: '+sLoja+#13+
                   'Item: '+sItem+#13+
                   'Padrão atual: '+grdAprovacao.Columns[9].Field.AsString+#13+
                   'Novo padrão: '+grdAprovacao.Columns[10].Field.AsString,'Pergunta.') then
         begin
             Exit;
         end;
        end;
      ...
      begin
        ...
        begin
         if not Pergunta('Deseja realmente aprovar esta quantidade?'+#13+
                   'Loja: '+sLoja+#13+
                   'Item: '+sItem+#13+
                   'Padrão atual: '+dtmSistema.qryGridSolicitacao.FieldByName('est_padrao').AsString+#13+
                   'Novo padrão: '+sQtd,'Pergunta.') then
         begin
             bGravar := False;
         end;
        end;
...
begin
    ...
    //não é tratado exception pois aqui nao pode dar erro e precisamos saber se acontecer algum
    if RedeLoja = '70' then
    begin
...
begin
    ...
    begin
        Informacao('Rede Inválida!      '+#13+#13+
                   'Rede deve ser:      '+#13+
                   '10 - GRAZZIOTIN     '+#13+
                   '30 - PORMENOS       '+#13+
                   '40 - FRANCO         '+#13+
